...
if len(sys.argv) != 3:
    ...
    print "transformations to choose from: vegetarian, meatify, healthy, unhealthy, " \
          "chinese_to_italian, italian_to_chinese, american_to_middle_eastern, middle_eastern_to_american, " \
          "mexican_to_soul_food, soul_food_to_mexican, french_to_indian, indian_to_french"
    quit()
...
sup_methods = ['chop', 'grate', 'stir', 'shake', 'mince', 'crush', 'squeeze', 'sprinkle', 'season', 'dissolve', 'peel',
               'seed', 'mix', 'grind', 'ground', 'dry', 'cure', 'grease', 'degrease', 'dredge', 'dehydrate', 'dried',
               'brush',
               'fold', 'cut', 'stuff', 'ferment', 'flambe', 'foam', 'can', 'paste', 'steep', 'infuse', 'dissolve',
               'juice',
...
for each in sup_methods:
    variations.extend(
        [each + 'd', each + 'ed', each + each[-1] + 'ed', each + 'ing', each + each[-1] + 'ing', each[:-1] + 'ing'])
...
# list of methods
main_methods = ['saute', 'sautee', 'broil', 'boil', 'poach', 'grill', 'pan-fry', 'pan fry', 'stir-fry', 'stir fry',
                'cook', 'deep-fry', 'deep fry', 'roast', 'bake', 'sear', 'simmer', 'steam', 'blanch', 'braise', 'stew',
                'chill', 'freeze', 'refrigerate', 'toast', 'heat', 'fry', 'caramelize', 'charbroil', 'glaze', 'deglaze',
                'microwave', 'nuke', 'frozen']
...
for each in main_methods:
    variations_main.extend(
        [each + 'd', each + 'ed', each + each[-1] + 'ed', each + 'ing', each + each[-1] + 'ing', each[:-1] + 'ing'])
main_methods = main_methods + variations_main

cookingTools = ['pan', 'grater', 'rack', 'whisk', 'paper towel', 'tong', 'spatula', 'knife', 'oven', 'microwave',
                'skillet', 'saucepan', 'pot', 'bowl', 'plate'
    , 'baking sheet', 'stovetop', 'knife', 'colander', 'strainer', 'aluminum foil', 'rolling pin', 'ladle', 'peeler',
                'pastry bag'
    , 'blender', 'food processor', 'juicer', 'wok', 'dutch oven', 'convection oven', 'parchment paper', 'kettle',
                'sheet pan'
...
if transformation == 'vegetarian':
    ...
    for ing in ingredientObjects:
        for meat in meats:
            if meat in getattr(ing, "name") or meat + 's' in getattr(ing, "name") or meat + 'es' in getattr(ing,
                                                                                                            "name"):
    ...
    cuisine_dict = {}
    if transformation == 'chinese_to_italian':
        cuisine_dict = foodList.chineseToItalian
    elif transformation == 'italian_to_chinese':
        cuisine_dict = foodList.italianToChinese
    ...
    for ing in ingredientObjects:
        for cuisine_food in cuisine_foods:
            if cuisine_food in getattr(ing, "name") or cuisine_food + 's' in getattr(ing,
                                                                                     "name") or cuisine_food + 'es' in getattr(
                    ing, "name"):
                cuisine_foods_used.append(cuisine_food)
                cuisine_foods_used.append(cuisine_food + 's')
                cuisine_foods_used.append(cuisine_food + 'es')
                break
    ...
    for ing in range(len(ingredientObjects)):
        ...
        for m in cuisine_foods_used:
            if m in i:
                ...
                if m[-1] == 's' and m[-2] == 'e':
                    newName = cuisine_dict[m[:-2]]
                elif m[-1] == 's':
                    newName = cuisine_dict[m[:-1]]
                else:
                    newName = cuisine_dict[m]
                for loc in locations:
                    for j in range(loc[1] - loc[0] + 1):
        ...
        if newName is not None:
            ...
            ingredientObjects[ing].clearPreparation()
            ingredientObjects[ing].clearDescriptor()
            # print getattr(ingredientObjects[ing], "name")
